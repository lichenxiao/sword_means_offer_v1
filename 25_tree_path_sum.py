...
def tree_path_sum(tree_root, sum, now_sum=0, now_path=[]):
    """
    树的路径的和，对与每个节点都算到和里，可以使用前序遍历方式
    """
    if not tree_root:
        return
    now_sum += tree_root.data
    now_path.append(tree_root.data)

    if not tree_root.left and not tree_root.right:
        if now_sum == sum:
            print ' '.join([str(i) for i in now_path])
    if tree_root.left:

        tree_path_sum(tree_root.left, sum, now_sum, now_path)
    if tree_root.right:
        tree_path_sum(tree_root.right, sum, now_sum, now_path)
